...
export const ClientActionMenu: React.FC<ClientActionsMenuProps> = ({ clientId, onSuccess }) => {
  ...
  const menuItems = [
    { value: "view", icon: <FiEye />, label: "Подробнее", onSelect: () => navigate(AppRoutes.CLIENT_DETAILS.replace(":id", clientId)) },
    { value: "edit", icon: <FiEdit2 />, label: "Изменить", onSelect: () => setOpenDialog(ClientDialogType.UPDATE) },
    { value: "delete", icon: <FiTrash2 />, label: "Удалить", onSelect: () => setOpenDialog(ClientDialogType.DELETE) },
  ];
